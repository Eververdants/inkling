...
**inkling** 是一个 AI 编程助手用的技能。你没想好做什么项目时，它会用 5 个阶段、约 10 个问题，帮你把一个模糊的念头，变成一份可以照着开干的完整提案。

支持 Claude Code、Cursor、Codex、Windsurf、Gemini CLI、OpenCode 等所有兼容 Anthropic SKILL.md 格式的 agent。
...
AI 会问你约 10 个问题，贯穿 5 个阶段。结束后提案文件会保存在 `<你的项目>/docs/inkling/`。

## 安装方式

### 插件市场（Claude Code CLI）
...
支持 40+ 种 agent，自动检测你用的工具并装到正确位置，不需要手动找路径。
...
或者 clone 整个仓库（SKILL.md 在 `skill/SKILL.md`）：
...
把 `skill/` 文件夹复制到对应 agent 的 skills 路径：
...
把整个仓库 clone 到 OpenCode 的 skills 目录。OpenCode 会自动发现任意层级的 SKILL.md 文件，所以嵌套的 `skill/` 目录没有问题：

```bash
git clone https://github.com/Eververdants/inkling.git ~/.opencode/skills/inkling
```

文件结构是 `~/.opencode/skills/inkling/skill/SKILL.md`，无需修改配置文件。重启 OpenCode 后即可使用。

## 你会得到什么

一份 6 板块的 markdown 提案，成熟到可以直接开工：

```
<你的项目>/docs/inkling/2026-06-24-<slug>-idea.md
```
...
每次对话分为 5 个阶段，每个阶段有明确的目标和退出条件。你掌握节奏——可以前进、深入追问，也可以回退修改。
...
**新增示例：** 创建 `examples/<project-slug>.md`，包含完整的 6 板块提案，末尾附加 100–200 字的对话复盘。
